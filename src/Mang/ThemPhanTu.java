package Mang;
//Arrys.toString chuyển giá trị mảng thành chuỗi cho nhanh khỏi kiểm tra for
//sort ;sắp xếp
...
public class ThemPhanTu {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            System.out.printf("a[i]=", i);
            phanTu[i] = scanner.nextInt();
        }
        System.out.print("Nhập phần tử chèn = ");
        int giaTriChen = scanner.nextInt();
        // sắp xếp dãy số theo thứ tự tăng dần
        sapXepThuTu(phanTu);
        System.out.print("Sắp xếp mảng tăng dần: ");
        show(phanTu);
        System.out.printf("\nChèn phần tử x vào mảng.", giaTriChen);
        phanTu = insert(phanTu, giaTriChen);
        System.out.print("\nMảng sau khi chèn: ");
        show(phanTu);
    }
     ...
     */
    public static void sapXepThuTu(int[] arr) {
        int temp = arr[0];
